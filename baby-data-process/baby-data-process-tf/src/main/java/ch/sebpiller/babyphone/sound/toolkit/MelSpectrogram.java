...
public class MelSpectrogram implements PitchDetectionHandler {
    ...
    AudioProcessor fftProcessor = new AudioProcessor() {
        ...
        final FFT fft = new FFT(bufferSize); // Instantiates FFT with the defined buffer size.
        final float[] amplitudes = new float[bufferSize]; // Array to store the computed amplitudes for each frequency bin.
    ...
    public static BufferedImage extractSpectrogramFromAudioFile(byte[] f, AudioFormat format) {
        var melGram = new MelSpectrogram(); // Create a new instance of MelSpectrogram.
        melGram.setOutputFrameWidth(Width); // Sets the standard mel spectrogram width.
        melGram.setOutputFrameHeight(Height); // Sets the standard mel spectrogram height.

        try {
            return melGram.convertAudio(f, format); // Processes the audio file and generates the spectrogram.
        } catch (Exception e) {
    ...
    public BufferedImage convertAudio(byte[] audio, AudioFormat format) throws UnsupportedAudioFileException {
        var dispatcher = AudioDispatcherFactory.fromByteArray(audio, format, bufferSize, overlap); // Creates a dispatcher for file-based audio sources.
