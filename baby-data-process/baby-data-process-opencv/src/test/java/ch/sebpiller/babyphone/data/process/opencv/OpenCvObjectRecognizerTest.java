...
@Slf4j
@Disabled
class OpenCvObjectRecognizerTest {

    private static OpenCvObjectRecognizer openCvObjectRecognizer;

    @BeforeAll
    static void setup() {
        openCvObjectRecognizer = new OpenCvObjectRecognizer(Map.of(new Detector(), new CascadeClassifier(Objects.requireNonNull(OpenCvObjectRecognizerTest.class.getResource("/haarcascades/haarcascade_frontalface_alt.xml")).getPath())));
...
//        var res = recognitionService.annotateImage(src);
        var out = openCvObjectRecognizer.detectAndWrite(bi, Optional.empty(), x -> true);
